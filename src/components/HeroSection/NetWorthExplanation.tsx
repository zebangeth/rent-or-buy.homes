  ...
  return (
    <>
      <p className="text-xl md:text-2xl leading-relaxed text-dark-800 mb-1 font-medium">
        If you buy a {formatCurrency(buyInputs.propertyPrice)} home today (with {buyInputs.downPaymentPercentage}% down
        payment) and it grows to <span className="font-bold text-primary-700">{formatCurrency(futureHomeValue)}</span>{" "}
        in <span className="font-bold">{appSettings.projectionYears}</span> years, your net worth would be{" "}
        <span className="font-bold text-primary-700 border-b-2 border-primary-300">{formatCurrency(buyNetWorth)}</span>.
      </p>

      <p className="text-xl md:text-2xl leading-relaxed text-dark-800 mb-5 font-medium">
        If you rent for {formatCurrency(rentInputs.currentMonthlyRentAmount)}/month and invest the difference, your net
        worth would be{" "}
